...
in
{
  ...
  checks = {
    ...
    tests-caddy = pkgs.testers.runNixOSTest (import ../tests/caddy.nix);
        # Disabled: Hyprland v0.52.1 flake incompatible with nixpkgs-unstable (xorg scope in override)
    ...
    nixos-eval-telfir =
      let
        ...
        configName = inputs.self.nixosConfigurations.nixos.config.system.name;
      in
