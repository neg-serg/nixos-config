...
1. [ ] Create `packages/rofiw/` — Rust binary replacing `files/rofi/rofi-wrapper.sh` (123 LOC bash)
   ...
   - `serde_json` for reading Quickshell `.theme.json` (panel offsets) and `hyprctl -j monitors`
     (scale)
   - Pure Rust offset math replacing `awk`/`sed`/`grep`
   - `exec rofi` at the end — hyprctl stays as subprocess
   - Wire: `packages/overlays/gui.nix`, replace `@ROFI_BIN@`/`@JQ_BIN@`/`@HYPRCTL_BIN@` with clap
     env defaults
   - Acceptance: `rofiw -theme catppuccin -show drun` works

1. [ ] Create `packages/pwroute/` — Rust binary replacing `modules/hardware/audio/hdspe/pw-route.sh`
   (190 LOC zsh)

   - `clap` subcommands: `{aes,an,spdif,phones}` for direct routing, `toggle` (cycle aes↔an),
     `current`, `status`, `list` (JSON)
   ...
   - Wire: `packages/overlays/media.nix`, update `hdspe/default.nix` to use `pkgs.pwroute` instead
     of `writeScriptBin`
...
1. [ ] Create `packages/amnezia-tun/` — Rust binary replacing `amnezia-import-tun-config.sh` (136
   LOC bash + Python)
   ...
   - Wire: `packages/overlays/tools.nix`, update `vpn-scripts/default.nix` to drop python3
     dependency if unused
   - Acceptance: `amnezia-tun import` decodes config, `amnezia-tun check` verifies

1. [ ] Delete old scripts after verification:
...
1. [ ] Update `hdspe/default.nix` — replace `writeScriptBin "pw-route"` with `pkgs.pwroute`, remove
   zsh + gawk deps

1. [ ] Update `vpn-scripts/default.nix` — replace bash+python script with `pkgs.amnezia-tun`

1. [ ] Check quickshell Audio.qml references — ensure `pwroute` binary name matches (currently calls
   `pw-route`)

## Final Verification Wave

F1. [ ] `cargo build --release` in each of 3 packages succeeds F2. [ ]
`nix build .#rofiw .#pwroute .#amnezia-tun` succeeds F3. [ ] `nh os switch /etc/nixos` rebuilds
without errors F4. [ ] `pwroute list` returns valid JSON F5. [ ] `amnezia-tun show-path` prints
~/.config/sing-box-tun/config.json
