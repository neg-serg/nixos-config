...
**intent**: clear **review_required**: false **status**: awaiting-approval
...
`get_position()` | `{x=..., y=...}` | `x, y` | | `get_window_size()` | `{width=..., height=...}` |
`width, height` | | `get_mouse_pos()` | `{x=..., y=...}` | `x, y` |

The fork switched from Lua tables to tuples. The init.lua safe wrapper `get_pos()` was written for
the upstream table API. On the fork:
...
**Result**: ALL panning starts from (0,0), never from current position. After first pan, position
becomes (-50,0) etc. But if image fits window, fixup_position() immediately resets to center.

### Secondary issues found:

1. Slideshow panning `p.x-50` where `p` is a number → Lua error (caught, silently breaks)
1. `Ctrl-Shift-comma` on US layout → XKB produces `less` keysym, not `comma` → binding never matches
1. `bind_reset()` removes zoom (`+`/`-`), reset (`BackSpace`), mouse scroll → never restored
1. Gallery missing `Left`/`Right` arrow bindings
1. `fixup_position()` with `free_move=false` (default) forbids panning when image fits window
