...
| C2  | dead quickshell assets                                                     | 12 shader files (`diag`, `moon`, `seam`, `seam_fill`, `seam_tint`, `sun` — `.frag` + `.qsb` each; `panel_tint_mix` and `wedge_clip` are live), `illustrations/moon_nasa.jpg` (139 KB), `Theme/.theme-light.json` (6.2 KB), `files/winapps/windows.svg`, and exactly four greeter icons (`bell-slash`, `bluetooth-x`, `wifi`, `wifi-slash` — enumeration included the two template-string icon references, so `play`, `pause`, `shuffle` are live); `icons.txt` documents the phosphor edits and should stay | delete on request (~150 KB, 16 files)                  |
...
| C4  | `hal.html` (115 528 B, untracked, repo root)                               | still untracked and untouched by this pass; provenance unknown                                                                                                                                                                                                                                                                                                                                                                                                                                              | delete or move on request                              |
