...
| nvim `lusty/native_source.lua` for the three source pickers           | −71 in the pickers (+31 net) | 32 049-line mock-vim dump identical to the pre-change baseline (two runs byte-identical; the height 6→5 mutation moves 6027 lines), 16/16 headless smoke, `check-lusty-smoke.sh` exit 0, `loadfile` ×4 and the real require chain under `nvim --clean --headless` |
| quickshell: `CustomTrayMenu` is now a 17-line `SubmenuHost`           | −163                         | Node harness replays showAt/hideMenu/Escape/Return/model/implicitHeight for root and nested menus — identical dumps; `qmllint` exit 0 on both files and on all 166 files (0 errors, warnings 9826→9733); the repo QML gate passes                                 |
| 12 (R5) — 14 per-domain aggregators via `neg.importDomain`            | −74                          | standalone `nix-instantiate --eval --strict --json` of all fourteen modules before and after: byte-identical import lists                                                                                                                                         |
...
Running total since `b3e8f158b` (the dead-code audit commit), measured at `2b7040fe5` — the last
code commit of the pass, before this file's own update: **41 commits, 296 files, +3 799 / −7 208 →
net −3 409 lines.** The reduction is coarser than the original estimate because the survey counted
*duplication removed*, while a shared, documented module also adds lines; the sections below keep
both numbers where they differ.
...
Every remaining line of the ranked tables was re-checked against the tree at the end of the pass:

| Item                                                  | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 12 (R5) — the 14 per-domain `default.nix` aggregators | **done** in `bfb430fb5` (−74): `neg.importDomain` sits next to `importDir`, all fourteen aggregators are one line (`{ neg, ... }: neg.importDomain ./.`), and the fourteen import lists were dumped as JSON before and after — byte-identical. The cosmetic half of the item ("13 flat four-line ones by formatting") was skipped.                                                                                                                                          |
| Tier 3 — `systemdUser` on `config.lib.neg` (~23)      | **already implemented** before this pass: the helper lives in `modules/core/neg.nix` and is used by nine modules (`emotion-diary`, `autoclick`, `user-services`, six `dsh-*` apps); both hand-rolled units already go through `mkUserService`. Re-verified by diffing the built `/etc/systemd/user` trees of generations 1860 and 1861: eleven units, identical store paths and hashes, `ydotoold` 7/7 and `at-spi-dbus-bus` 8/8 fields.                                    |
| Tier 3 — `neg.readFile` (~17)                         | **already implemented**: `config.lib.neg.readFile = p: builtins.readFile (path p)`, used by seven modules. The "drop redundant `config.`" half is impossible — the `neg` specialArg has no `path`, so the qualifier is load-bearing at those sites, which is why they stayed.                                                                                                                                                                                               |
| Tier 3 — `dsh-plugin-kit` (~100)                      | **not applicable.** Re-checked: the `pluginMessage` helper the survey counted ×11 no longer exists in the tree, and the one genuinely duplicated block — the line hash — carries `MUST stay byte-identical to dsh-hashline (lineHash)` (`dsh-read-tags/lib/index.js:30`), so a shared import would break the invariant the two plugins rely on. What remains is `sessionCwd(exec)` in three plugins (~20 lines), and the caretaker copies each plugin directory standalone. |
| Tier 3 — `vp-*` pickers (~25)                         | dropped: no such files exist in the tree.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 4 — shared `test-harness.mjs` for the small dsh tests | closed earlier as tried-and-reverted (net −1).                                                                                                                                                                                                                                                                                                                                                                                                                              |

With that, the only open entries are the owner decisions below.

## Owner decisions — evidence for the open items
...
### C2 asset verification

Requested check: are those assets really dead? Each candidate was tested with three independent
methods — an exact-filename and stem search across the whole repository, the loading mechanism of
its asset class, and a runtime probe wherever the asset could be removed without touching the live
session.

| Asset                                                                                 | Loading mechanism                                                                                                                                                                                                                                                   | Static result                                                                                                                                          | Runtime probe                                                                                                                  |
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| 12 shaders `diag`, `moon`, `seam`, `seam_fill`, `seam_tint`, `sun` (`.frag` + `.qsb`) | `fragmentShader: Qt.resolvedUrl("../shaders/<name>.frag.qsb")` — the six sites in `Bar/Bar.qml` bind only `panel_tint_mix` and `wedge_clip`, the greeter binds `masked_overlay`; `compile_shaders.sh` compiles whatever sits in the directory, there is no manifest | zero hits for the six stems outside the shader files themselves                                                                                        | all twelve moved out of `~/.config/quickshell/shaders`, shell restarted: warning set identical, not one shader-related message |
| `illustrations/moon_nasa.jpg`                                                         | the calendar art is built as `../../art/calendar/` + (month+1) + `.svg` (1..12.svg exist); wallpapers are picked from `~/pic/wl`                                                                                                                                    | only the two audit documents mention it                                                                                                                | the whole `illustrations` symlink moved aside, shell restarted: nothing complained                                             |
| `Theme/.theme-light.json`                                                             | theme parts are the `Theme/*.jsonc` files listed in `manifest.json`; `collectJsonFiles` skips every dotfile (`entry.name.startsWith('.')`)                                                                                                                          | `theme-light` / `themeLight` / `theme_light` have no code hit anywhere; the deployed copy is stale (Jul 29) while `.theme.json` is regenerated (Sep 1) | file moved out of the deployed `Theme/`, shell restarted: nothing complained                                                   |
| `files/winapps/windows.svg`                                                           | winapps registers each app with `ICON="$APPS_DIR/$APP/icon.svg"` (`winapps.sh:32`) and `Icon=@APPDIR@/@APPNAME_V2@/icon.svg` (`desktop-entry.in:7`)                                                                                                                 | zero hits repository-wide; the file sits outside every app directory                                                                                   | not applicable — it is only reachable by name, and no code names it                                                            |
| 4 greeter icons `bell-slash`, `bluetooth-x`, `wifi`, `wifi-slash`                     | greeter icons are literal paths in the greeter QML plus two template forms (`pause`/`play`, `shuffle`/`shuffle-off`)                                                                                                                                                | the full enumeration of `icons/…` references yields 23 used names; these four appear nowhere; `icons.txt` is provenance prose, not a manifest          | not attempted: exercising the greeter means restarting greetd, which would kill the session                                    |
