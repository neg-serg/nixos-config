...
### Использование Comss.one DNS (опционально)

[Comss.one](https://www.comss.ru/page.php?id=7315) — российский приватный DNS-сервис с поддержкой
DNS-over-TLS. Может быть полезен, когда Cloudflare/Quad9 заблокированы или работают нестабильно.

Для включения Comss DNS в качестве апстрим-резолвера:

```nix
servicesProfiles.unbound = {
  enable = true;
  useComssDns = true;  # заменяет Cloudflare/Quad9 на Comss.one
};
```

> **Примечание:** Comss DNS отключён по умолчанию. При включении он заменяет `dotUpstreams` на
> серверы Comss DoT (92.223.109.31, 93.115.24.204, 91.230.211.67 на порту 853).

### Сбор метрик

Чтобы собирать метрики, включите экспортер, скрейп‑задачу и добавьте источник Prometheus в Grafana:
