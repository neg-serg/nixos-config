...
`nh` is a wrapper. To profile the actual work, we first need to replicate what it does using raw Nix commands.
...
If this command is fast (< 1s), the bottleneck is **`nh` itself** (likely its Git operations).
If this command is slow (~10s), the bottleneck is **Nix evaluation**.

## 2. Profiling `nh` (Git/IO Bottleneck)

If raw Nix is fast, `nh` is likely spending time copying/fetching the git tree.
Use `strace` to see what `nh` is doing.
...
Analyze `trace.log` (or Use `strace -c` for summary) to see if it's walking the entire `.git` directory or copying thousands of files.
...
1.  **Run with Callgrind:**
    We profile `nix-instantiate` because that's where evaluation happens.
...
1.  **Git Input Fetching**: If using `git+file://`, Nix creates a git archive every time. Large `.git` folder = slow.
    *   *Fix*: Use `path:.` instead of `git+file:.` (if pureness isn't strict requirement) or optimize git repo.
2.  **Slow Imports**: Importing huge lists of packages or recursively reading dirs (`builtins.readDir` on large trees).
3.  **IFD (Import From Derivation)**: If evaluation triggers builds (e.g., fetching a plugin via `fetchFromGitHub` *during eval* to get its `default.nix`).
