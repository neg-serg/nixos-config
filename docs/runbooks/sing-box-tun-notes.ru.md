...
- Секрет конфига: `/run/secrets/vless-reality-singbox-tun.json` (от sops:
  `secrets/home/vless/reality-singbox-tun.json.sops`, раскатывается sops-nix при наличии файла).
- Сервис: ручной запуск `systemctl start|stop|restart sing-box-tun`. Юнит прописывает policy
  ...
  - При старте сохраняет прежний default из table 200 в `/run/sing-box-tun/prev-default-route`, при
    остановке возвращает его (если был) и сбрасывает настройки DNS.
- Требования: установленный `sing-box` (в `environment.systemPackages`), секрет должен быть
  развёрнут (sops-nix), нужен `CAP_NET_ADMIN` (root/systemd).
