...
Этот документ описывает алгоритм перевода текущей конфигурации (где задействован только
`sops-nix`) на `vaultix`. Предполагается, что модули подключаются из конфигурации NixOS,
поскольку `vaultix` предоставляет модуль именно для NixOS и собственные CLI-инструменты.
