...
Этот хелпер упрощает запуск окружения для
[DeepFaceLab_Linux](https://github.com/nagadit/DeepFaceLab_Linux) в контейнере Ubuntu на NixOS.

## Ограничения

- Видеокарта: AMD (RX 7900 XTX), поэтому DeepFaceLab работает только в CPU-режиме, без
  CUDA/ROCм-ускорения.
...
`~/vid` уже привязан к большому разделу `/zero/vid`, поэтому сюда можно безопасно складывать тяжёлые
данные.
...
1. Выполнить шаги по настройке Python/Conda и установке зависимостей из README.
1. Запускать скрипты DeepFaceLab, используя:
   - `/workspace` для кода;
