...
- Чтобы запускать нативные Wayland-клиенты внутри Gamescope (или даже вложенный Hyprland), используйте `--expose-wayland`.
- Команда: `game-run gamescope --expose-wayland -- %command%`
- Применение: запуск приложений, требующих Wayland-сокет (например, некоторые SDL3 приложения или инструменты разработки), или для тестов.
- Примечание: Экспериментально. Возможны проблемы с вводом/фокусом. Не требуется для обычного гейминга в Steam/Proton.
