...
| Level                        | Verdict                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Client plugin (fork package) | **Recommended; shipped locally.** Registers `read_hashline` + `hashline_edit` as new tool names (no `read` conflict), reuses `ctx.fs`, `ctx.waterfall("fs/write-intent")`, `ctx.emit("fs/observed")`, and the `dsh-sandbox` escalation schema pattern. The fork (`dsh-web-ui`, removed in 2026-09) no longer carries `packages/dsh-hashline/`; the tool lives in `modules/user/nix-maid/apps/dsh-hashline/` and is wired by `modules/user/nix-maid/apps/dsh-terminal-plugins.nix`. |
| Fork patch (`dsh-tool-fs`)   | Needed **only** to put tags inside the built-in `read`. That is a first-party upstream bundle; it used to ship via the Nix `packages/dsh/web-ui-en` scope symlink, which was removed with the dsh web GUI in 2026-09, so patching it now means a build-time/scope-level override and re-doing it on upstream upgrades. Optional phase 2.                                                                                                                                           |
