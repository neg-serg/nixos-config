...
## Серверные slash-команды без браузерной части

Часть своих команд — чисто host-плагины (только `apply` на сервере, без `lib/client.js`): живут
прямо в `/etc/nixos/modules/user/nix-maid/apps/<name>/` и ставятся ensure-скриптом по паттерну
`dsh-mode.nix` (копия файлов в профиль + insert-строка в `cordis.patch.yml`).

| Пакет               | Команды                                                       |
| ------------------- | ------------------------------------------------------------- |
| `dsh-mode`          | `/mode` — список/смена агент-пресета по умолчанию             |
| `dsh-session-tools` | `/rename`, `/status`, `/remember`, `/forget` — утилиты сессии |

Клиентские popup-команды `/fork`, `/new`, `/goto`, `/model`, `/help` — build-time инъекция в
`packages/dsh/web-ui-en/patch.py` (паттерн commands anchor).
