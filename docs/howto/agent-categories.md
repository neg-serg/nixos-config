...
Port from **oh-my-opencode (oh-my-openagent)**. Idea: delegate by intent, not by model name.
A model name creates distributional bias — the model behaves differently when it thinks of itself
as "gpt-X" or "claude-Y"; a category instead describes WHAT needs to be done, and the harness picks
the model.
