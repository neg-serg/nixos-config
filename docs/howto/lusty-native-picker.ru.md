...
- Клавиши: Enter/Tab, C-t, C-o/C-v, C-n/C-p, C-f/C-b (колонки), C-l (long-режим с метаданными), C-s
  (сортировка name/ext/size/time), C-u (очистить), Esc/C-c/C-g (отмена), C-w (вверх по каталогу),
  точка (показать скрытые), RU-раскладка без переключения.
