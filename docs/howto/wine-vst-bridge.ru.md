...
> Исследование 2026-08-20, собрано субагентами по первоисточникам (GitHub, nixpkgs 26.05, исходники Carla).
> Нерешённые места помечены [unverified] — требуют живой проверки на odin.
> Контекст: NixOS 26.05, wineWow64Packages.stable (wine 11), PipeWire (pw-jack, без jackd),
> Carla из nixpkgs (JACK-only, wine-мост НЕ собран), CLI carlactl для headless-роутинга нативных плагинов.
...
*CLI-оценка: 0 = нет, 1 = есть CLI-конвертация, но нужен GUI-хост, 2 = скриптуемый CLI-поток, 3 = чистый CLI end-to-end.

## Рекомендация для odin

1. **Быстрый путь (минимум работы): yabridge.** Уже установлен (wine 9.21 пинится nixpkgs).
   `yabridgectl add "<префикс>/drive_c/Program Files/Common Files/VST3"` (и `VstPlugins` для VST2) → `yabridgectl sync` →
   `carlactl list` / `carlactl run vst3:Name`. Риски: 32-бит VST не поддерживаются; винная версия хоста (9.21) отличается от
   префиксов wineapps (11.0) — проверить на одном плагине; yabridgectl требует login-shell (NIX_PROFILES) или `--path`.
2. **Полная интеграция с carlactl: Carla wine bridge.** Потребует overlay: `carla.overrideAttrs` + `pkgsCross.mingwW64/mingw32.buildPackages.gcc`
   + `make win32 win64 wine32 wine64`, бинари в `$out/lib/carla`. Сборка [unverified] — рискованная, но даёт чистый CLI-роутинг
   через тот же C API (автодетект префикса, libmagic PE-типизация, wineasio не нужен).
3. **Для standalone-плагинов:** wineapps-префикс + pipeasio (надо запаковать в nixpkgs-оверлей) или wineasio (есть) —
   полностью скриптуемо без правок Carla.

## Ключевые источники
- Carla wine bridge: https://github.com/falkTX/Carla (INSTALL.md:101–118, CarlaPluginBridge.cpp, CarlaBackend.h:1493–1518),
  nixpkgs issue #324094, https://kx.studio/Applications:Carla. Осторожно: открытый регресс `carla --no-gui` #1828 (2.5.7+).
- yabridge: https://github.com/robbert-vdh/yabridge (README, docs/architecture.md, tools/yabridgectl/README.md),
  nixpkgs: pkgs/by-name/ya/yabridge + yabridgectl (wine 9.21, -Dbitbridge=false), issues #300755, #399465.
...
- wineasio: https://github.com/wineasio/wineasio ; pwasio: https://github.com/golfiros/pwasio ; pipeasio: https://github.com/M0n7y5/pipeasio
- MrsWatson (форк): https://github.com/adamnemecek/MrsWatson ; VSTHost/SAVIHost: https://syntheway.com/Hermann_Seib_VSTHost_v1.53_SAVIHost_v1.41.htm
- Винные воркфлоу для VST: https://wiki.nixos.org/wiki/Electric_guitar_interface_setup

*Открытые вопросы: (1) Carla bridge JACK под pw-jack (LD_LIBRARY_PATH чистится, jackbridge-wine.dll резолвит libjack) [unverified];
