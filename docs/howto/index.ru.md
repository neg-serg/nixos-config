# How-to и справка

Здесь лежат точечные заметки: пины, хоткеи, инвентарь клиентов и другая справка, которая не тянет на
пошаговый runbook.

- Заметки по плагину Hyprland hy3: [hyprland-plugin.md](./hyprland-plugin.md)

- Шпаргалка по hxtools: [hxtools.md](./hxtools.md), [hxtools.ru.md](./hxtools.ru.md)

- Бенчмарк дедупликации /nix: [nix-dedup-vdo.en.md](./nix-dedup-vdo.en.md),
  [nix-dedup-vdo.ru.md](./nix-dedup-vdo.ru.md)

- Горячие клавиши Swayimg: [swayimg-hotkeys.md](./swayimg-hotkeys.md),
  [swayimg-hotkeys.ru.md](./swayimg-hotkeys.ru.md)

- Настройка Valve Index VR: [valve-index-vr.md](./valve-index-vr.md),
  [valve-index-vr.ru.md](./valve-index-vr.ru.md)

- DeepFaceLab через Docker (RU): [deepfacelab-docker.ru.md](./deepfacelab-docker.ru.md)

- TODO по запасным алиасам Aliae: [aliae-fallback-todo.md](./aliae-fallback-todo.md)

- Справочник опций/модулей: [modules.md](./modules.md)
