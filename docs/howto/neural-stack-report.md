...
Check date: **2026-08-28 … 2026-09-05**. Host: odin (NixOS, AMD RX 9070 XT, Ryzen 9 9950X3D, 60 GB
RAM). Goal — verify that all installed neural tools really work after the build, fix what is broken,
document the status. All models live in `/zero/ai/**`.
...
- Wrapper: `packages/local-bin/bin/rembg` (venv `venv-rembg`). Default is `-m u2net` unless the user
  passed their own `-m` (bria-rmbg-2.0 was downloaded from a blocked GitHub).
...
1. **rembg pulls bria-rmbg-2.0.onnx from GitHub** (blocked) → model downloaded via a socks5h proxy
   (`127.0.0.1:10808`) into `/zero/ai/imgproc/models/bria-rmbg/`; the wrapper defaults to u2net.
...
1. **venv-demucs** (bin→3.14, packages in 3.13): recreated on python3.13, CPU torch installed first
   (`pip install torch==2.8.0 --index-url https://download.pytorch.org/whl/cpu`), otherwise pip
   pulls CUDA torch + 2 GB of nvidia wheels.
...
- **Wrappers**: `packages/local-bin/bin/` (Nix-managed; test via the direct path from the repo —
  they land in `~/.local/bin` after `nh os switch`). `amt-generate` — symlink to
  `/zero/ai/music-ai/bin/`.
...
- `pic-ocr` NN engine (qwen3-vl) — one-off run when the "OCR NN" button in the screenshot toast is
  used.
- `venv-tags`/PANNs — orphan (no weights, python hangs on a nonexistent path) — a removal candidate.
- `colibri` — service off by default (user decision); manual recipe: see `docs/howto/local-llm.md`.
