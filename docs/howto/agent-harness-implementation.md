...
`compaction/*`, `todo/write`, `goal/change`. The auto-continuation pattern is `dsh-goal-round-driver`
(`agent.followup`). Todo = the last `todo/write` event (last-write-wins). Memento — the third-party
plugin `dsh-memento` (`ctx.provide('memory')`, SQLite `~/.dsh/dsh-memento/`).
...
1. ✅ **dsh-category-skill-reminder** — agent plane (`dsh-liangshen-fork/agent.cordis.yml`),
   skill catalog via `ctx.skills.snapshot`.
...
1. ✅ **dsh-boulder** — `agent/status` idle + todo projection + `agent.steer` with CONTINUATION_PROMPT
   (countdown 2s, backoff 5s×2, max 5, 5-minute pause, stagnation 3, compaction guard 60s) +
   toast-client.
...
- `.agent/workflows/notepads.md` — notepad system
  (learnings/decisions/issues/verification/problems), read before delegating, append after a
  task, pass to all subsequent subagents.
...
- Where do the neg-preset fork plugins live (`~/.dsh/profiles/web/` vs
  `modules/user/nix-maid/apps/`) — recon points at server plugins, but the fork source directory
  for new packages still needs to be chosen (for example `packages/dsh/server-plugins/`).
- Whether `dsh-memento` is patched as a third-party plugin in the profile or through a fork —
  to be clarified when implementing item 5.
