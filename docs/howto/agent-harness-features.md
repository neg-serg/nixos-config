...
Solution: every read line gets a tag `{line}#{hash}` (CID alphabet `ZPMQVRWSNKTXJBYH`, 2 characters);
edits reference the tags, and on a hash mismatch the edit is rejected before corruption.
...
- **todo-continuation-enforcer** (boulder): exact text in `agent-guards.md` §6; mechanics
  (countdown 2s, backoff 30s×2, max 5, 5-minute pause) — ✅ implemented by the dsh-boulder plugin.
- **preemptive-compaction**: preemptive compaction before the token limit — 📐 design ready:
  ...
  — `agent-micro-compaction.md`; implementation by the dsh-micro-compaction plugin is a separate
