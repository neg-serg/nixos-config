...
| `designs/rules-hooks.ru.md`                                                      | 4 маленьких хука: agent-usage-reminder (S), compaction-todo-preserver (S), rules-injector (M), category-skill-reminder (S–M); все — server plugin в пресете neg; порядок: usage → todo-preserver → rules → category                                                                                    | ✅ готов  |
...
| `designs/memory-pipeline.ru.md`                                                  | два готовых промпта (memory-extract, memory-consolidate) + интеграция с memento: триггеры, маппинг в треки/скиллы, бюджеты, approval-gate                                                                                                                                                              | ✅ готов  |
| `.agent/prompts/memory-extract.ru.md`, `.agent/prompts/memory-consolidate.ru.md` | готовые промпты (RU), извлечены из дизайна                                                                                                                                                                                                                                                             | ✅ в репо |
| `designs/boulder.ru.md`                                                          | boulder: поведенческая спецификация (countdown 2s, backoff 30s×2 max 5, пауза 5 мин, stagnation max 3, compaction guard 60s, полный список «когда НЕ вставлять»), интеграция (`packages/dsh-boulder/` в форке), skeleton: package.json + constants (точный текст) + index.ts (хост-логика) + client.ts | ✅ готов  |
