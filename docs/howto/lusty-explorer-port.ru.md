...
- Размеры float: ширина ~90% экрана, высота подстраивается под контент (таблица целиком + строка
  prompt), максимум ~80% высоты экрана; минимум строк — одна на запись (при малом числе). Окно
  центрируется. Нижняя строка — чистый prompt `>>` (+ текущий путь/запрос в файловом explorer), без
  подсказок.
