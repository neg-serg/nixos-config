...
- Таблица по умолчанию показывает несколько строк (минимум 4), а не всё в одну строку; в нижней
  строке-промпте при пустом запросе — подсказка: `"имя/"` — войти, `<C-w>` — вверх, `"."` — показать
  скрытые файлы.
