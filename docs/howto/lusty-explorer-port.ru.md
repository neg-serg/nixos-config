...
- Первая буква запроса обязана быть префиксом имени файла (не пути целиком): `c` не найдёт `pic.jpg`,
  хотя подстрока `c` внутри имени есть — чтобы набор не цеплял записи «по середине имени».
