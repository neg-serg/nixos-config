...
harness (rc.6), а `Session.append()` не умеет ставить маркер `ignorable` на событие. Ридер истории
отказывается открывать лог с неизвестным не-ignorable событием (`SessionFormatUnsupportedError`),
поэтому `tools.js` при загрузке плагина регистрирует три типа в общем сете (это отложенная
harness'ом поверхность регистрации событий сторонних плагинов). Логи, записанные ДО фикса,
отремонтированы вручную: событиям проставлен `"ignorable": true` (бэкапы — в
`~/.dsh/repair-backups/bash-live-ignorable/`).
