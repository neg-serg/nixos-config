...
>
> **Важно:** список инструментов сессии фиксируется при её создании. `bash_live` появляется только
> в **новых** сессиях, созданных после включения флага (после 12:09 19.08.2026); в старых сессиях
> его нет, даже после рестарта harness'а (проверено: сессия `session-e8d372df…` — все 74 события
> `tool/bash-live-*` несут `ignorable: true`, ридер её принимает и возобновляет после рестартов;
> её «смерть» 12:17 была зависшим ходом на `gavel_review`/длинном reasoning, а не форматом).
