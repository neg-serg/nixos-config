...
| **`builtins.readFile` in modules/media.nix**     | Reading files on every eval → IFD                                                               | Replace with `lib.cleanSourceWith` or path dependencies                            |
...
- `builtins.pathExists` with absolute paths outside the repo is an impure operation. Relative paths
  inside the source tree are cached fine by nix (they are deterministic). The only impure call was in
  `session/chat.nix` — replaced with a feature flag.

- `cores = 2` vs `cores = 4`: nix runs `max-jobs` parallel builds, each with `cores` threads. With 16
  physical cores / 32 threads:
