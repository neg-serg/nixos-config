...
| `Alt+Enter`        | Отправить текущую строку в Tidal       | `.tidal` файл |
| `Alt+Enter`        | Отправить **каждую строку** выделения  | visual mode   |
| `<leader><CR>`     | Отправить блок как одно выражение      | `.tidal` файл |
| `<leader>d`        | Заглушить паттерн под курсором         | `.tidal` файл |
| `<leader><Esc>`    | `hush` — заглушить всё                 | `.tidal` файл |
...
> `<leader><CR>` — наоборот, отправляет блок как одно выражение (для одного паттерна,
