...
-- Марковская цепь 1-го порядка (bd/sn/hh, матрица переходов; seed — вариации).
d1 $ markovGroove 16 42
d1 $ markovGroove 16 7    -- другой seed = другой грув
...
-- Хиндемит: Ряд 1/2 — ранг консонанса интервала (0..6) и сторона корня.
d1 $ note (cat [60, 64, 67]) # gain (fromIntegral (6 - hindemithRank 4) / 6)

-- Форте: интервальный вектор и нормализация pc-множества (печать значений).
--   print (icVector [0,1,4])    -- [1,0,1,1,0,0] (3-3)
--   print (pcCompact [7,11,2])  -- [0,4,9]
d1 $ note (cat (map (+60) (pcCompact [0,4,7]))) # sound "superpiano"

-- Мессиан: деши-талы (индийские ритмические ячейки).
d1 $ talaSound 1

-- Яворский: 18 ладов (тритоновые клетки разрешения).
d1 $ yavorsky 1

-- Холопова: параметрический комплекс — 5 бинарных флагов.
d1 $ holopova [1, 0, 1, 1, 0]

-- Тхаты Бхаткханде (шкалы раг).
d1 $ that 3   -- Bhairav

-- Макам: аджнас с четвертьтонами (дробные полутоны, 24-ТЕТ).
d1 $ slow 2 $ maqamBayati
d1 $ slow 2 $ maqamHijaz

-- Гамелан: микротоновые строи слендро/пелог.
d1 $ slow 2 $ slendro
d1 $ slow 2 $ pelog

-- Ксенакис: булева алгебра множеств высот (Herma).
d1 $ herma unionSet
d1 $ herma interSet
d1 $ herma diffSet

-- Танеев: подвижной контрапункт — два голоса на фиксированном интервале.
d1 $ taneev 4
```
...
многослойную сцену (37 строк, орбиты d1–d16): отправляй строки сверху вниз
...
Гласс → додекафония → тала → греческие стопы → марков → Мессиан → 1/f → Перл →
Хиндемит → деши-тала → Яворский → Холопова → тхата → макам → гамелан → Herma →
Танеев. В конце — `hush`.
