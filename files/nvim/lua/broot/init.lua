...
-- adapted locally to add a layout option so broot can open in a vertical or
-- horizontal split instead of a full-size floating window.
local M = {
  config = {
    broot_binary = "broot",
    layout = "float", -- float | vsplit | hsplit
    extra_args = {},
...
---@field layout "float"|"vsplit"|"hsplit"? How to open the broot window.
---@field extra_args string[]? Extra arguments to pass to broot
---@field config_files string[]? Paths to configuration files to pass to broot with the --conf argument. Values are passed to vim.fn.expand before being stored.
---@field default_directory string|(fun(): string?)|nil Default directory for broot.broot() when none is given.
...
---@class BrootOpts
---@field layout "float"|"vsplit"|"hsplit"? Overrides the global layout.
---@field extra_args string[]? Extra arguments to pass to broot.
---@field directory string? The directory to launch broot in.
...
function M.broot(opts)
  ...
  local layout = opts.layout or M.config.layout
  local buffer_id, window_id
  if layout == "vsplit" or layout == "hsplit" then
    -- Open a fresh split and run broot in it so the current buffer stays
    -- visible next to the file manager.
    vim.cmd(layout == "vsplit" and "vnew" or "new")
    ...
    vim.api.nvim_set_option_value("modified", false, { buf = buffer_id })
  else
    -- Full-size floating window (upstream behavior).
