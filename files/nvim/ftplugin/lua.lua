...
-- then falls back to a picker if nothing found.
...
local function follow_require()
  ...
  -- Picker fallback: use the last path component as the search query.
  local tail = mod_path:match('[^/]+$') or mod_path
  Snacks.picker.files({ search = tail, cwd = nav.project_root() })
end
