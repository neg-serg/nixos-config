...
* a font size change at an unchanged window size was not caused by resizing:
  ...
  back, so manual zoom survives later resizes (SLOP tolerates the pixel jitter
  a window manager may add while re-laying-out).
...
def _scale(boss: Boss, window: Window, data: dict[str, Any]) -> None:
    ...
    if now - _last_self_change.get(os_window_id, 0.0) < SELF_CHANGE_GRACE:
        return
    for stale_id in set(_anchors) - set(boss.os_window_map):
        _anchors.pop(stale_id, None)
        _first_seen.pop(stale_id, None)
        _last_pixels.pop(stale_id, None)
