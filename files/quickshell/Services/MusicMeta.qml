...
Item {
    ...
    function playerProp(keys) {
        ...
        for (var i = 0; i < keys.length; i++) {
            ...
            } catch (e) { /* metadata field unavailable */ }
        }
        // Try metadata dictionary variants
        var md = null;
        try { md = p['metadata'] || p['trackMetadata'] || p['meta'] || null; } catch (e) { console.warn("[MusicMeta.playerProp]", e); md = null; }
        if (md) {
            for (var j = 0; j < keys.length; j++) {
                ...
                } catch (e2) { /* metadata field unavailable */ }
            }
        }
        return undefined;
    }
        ...
        if (p) {
            try {
                ...
                if (md && typeof md === 'object') {
                    for (var k in md) {
                        try { out.push(String(md[k])); } catch (e) { /* metadata field unavailable */ }
                    }
                }
            } catch (e) { console.warn("[MusicMeta.mdAll]", e); }
            var directKeys = [
    ...
    function toFlatString(v) { if (v === undefined || v === null) return ""; try { if (Array.isArray(v)) return v.filter(function(x){return !!x;}).join(", "); } catch (e) { /* metadata field unavailable */ } return String(v); }
    ...
    function computeGenre() { var v = playerProp(["trackGenre", "genre", "genres", "xesam:genre", "xesam.genre"]); var s = toFlatString(v); if (s) return s; try { if (fileAudioMeta && fileAudioMeta.tags && fileAudioMeta.tags.genre) return toFlatString(fileAudioMeta.tags.genre); } catch (e) { /* metadata field unavailable */ } return ""; }
    function computeLabel() { var v = playerProp(["label", "publisher", "albumLabel", "xesam:publisher", "xesam:label", "xesam:albumLabel"]); var s = toFlatString(v); if (s) return s; try { if (fileAudioMeta && fileAudioMeta.tags) { if (fileAudioMeta.tags.label) return toFlatString(fileAudioMeta.tags.label); if (fileAudioMeta.tags.publisher) return toFlatString(fileAudioMeta.tags.publisher); } } catch (e) { /* metadata field unavailable */ } return ""; }
    function computeYear() { var v = playerProp(["year", "date", "releaseDate", "xesam:contentCreated", "xesam:year"]); var s = toFlatString(v); if (!s) return ""; try { if (/^\d{4}$/.test(s)) return s; var n = Number(s); if (!isNaN(n) && n > 1000) { if (n < 3000) return String(Math.floor(n)); var d = new Date(n); var y = d.getFullYear(); if (y > 1900 && y < 3000) return String(y); } var d2 = new Date(s); var y2 = d2.getFullYear(); if (y2 > 1900 && y2 < 3000) return String(y2); } catch (e) { /* metadata field unavailable */ } var m = s.match(/(19\d{2}|20\d{2})/); if (m) return m[1]; try { if (fileAudioMeta && fileAudioMeta.tags && fileAudioMeta.tags.date) { const y = String(fileAudioMeta.tags.date); const m2 = y.match(/(19\d{2}|20\d{2})/); if (m2) return m2[1]; } } catch (e) { /* metadata field unavailable */ } return ""; }
    function computeBitrateStr() { var v = playerProp(["bitrate", "audioBitrate", "xesam:audioBitrate", "xesam:bitrate", "mpris:bitrate", "mpd:bitrate"]); var s = fmtKbps(v); if (s) return s; try { if (fileAudioMeta && fileAudioMeta.bitrateKbps) return String(fileAudioMeta.bitrateKbps); } catch (e) { /* metadata field unavailable */ } return ""; }
    ...
    function isValidCodecName(s) { if (!s) return false; return /^[a-zA-Z][a-zA-Z0-9_ .-]*$/.test(String(s).trim()); }
    function computeCodec() { var v = playerProp(["codec","encoding","xesam:audioCodec","xesam:codec","mpd:codec"]); var s = toFlatString(v); if (s && isValidCodecName(s)) return prettyCodecName(s); var v2 = playerProp(["format","mimeType","mimetype"]); var s2 = toFlatString(v2); if (s2 && isValidCodecName(s2)) return prettyCodecName(s2); try { if (fileAudioMeta && fileAudioMeta.codec) return prettyCodecName(fileAudioMeta.codec); } catch (e) { /* metadata field unavailable */ } var all = mdAll(); var re = /(flac|alac|wav|aiff|pcm|mp3|aac|m4a|opus|vorbis|ogg|wma|ape|wv|dsd|dff|dsf)/i; for (var i2 = 0; i2 < all.length; i2++) { var str = all[i2]; var m = str.match(re); if (m) return m[1].toUpperCase(); } return ""; }
    function computeCodecDetail() { try { var parts = []; var base = trackCodec; if (!base && fileAudioMeta && fileAudioMeta.codec) base = prettyCodecName(fileAudioMeta.codec); if (!base && fileAudioMeta && fileAudioMeta.container) base = String(fileAudioMeta.container).toUpperCase(); if (base) parts.push(base); if (fileAudioMeta && fileAudioMeta.profile) parts.push(fileAudioMeta.profile); if (fileAudioMeta && fileAudioMeta.codecLong) { var upperBase = base ? base.toUpperCase() : ""; if (!upperBase || fileAudioMeta.codecLong.toUpperCase().indexOf(upperBase) === -1) { parts.push('(' + fileAudioMeta.codecLong + ')'); } } var out = parts.join(' '); return out; } catch (e) { console.warn("[MusicMeta.computeCodecDetail]", e); return trackCodec; } }
    function computeChannelsStr() { var v = playerProp(["channels","channelCount","xesam:channels","audioChannels","mpd:channels"]); var s = toFlatString(v); if (s) { if (/^1$/.test(s) || /mono/i.test(s)) return "1"; if (/^2$/.test(s) || /stereo/i.test(s)) return "2"; var m = String(s).match(/(\d+)\s*(?:ch|channels?)/i); if (m) return m[1]; if (/^\d+$/.test(String(s).trim())) return String(s).trim(); return ""; } try { if (fileAudioMeta && fileAudioMeta.channels) { var fs = String(fileAudioMeta.channels); if (/^1$/.test(fs) || /mono/i.test(fs)) return "1"; if (/^2$/.test(fs) || /stereo/i.test(fs)) return "2"; var m0 = fs.match(/(\d+)/); if (m0) return m0[1]; } } catch (e) { /* metadata field unavailable */ } var all = mdAll(); for (var i3 = 0; i3 < all.length; i3++) { var str2 = all[i3]; var m1 = str2.match(/(mono|stereo)/i); if (m1) return (/mono/i.test(m1[1]) ? "1" : "2"); var m3 = str2.match(/(\d+)\s*(?:ch|channels?)/i); if (m3) return m3[1]; } return ""; }
    function computeBitDepthStr() { var v = playerProp(["bitDepth","bitsPerSample","xesam:bitDepth","audioBitDepth","mpd:bitDepth"]); var s = toFlatString(v); if (s) { var m = String(s).match(/(\d{1,2})/); if (m) return m[1]; return ""; } try { if (fileAudioMeta && fileAudioMeta.bitDepth) { var bs = String(fileAudioMeta.bitDepth); var m0 = bs.match(/(\d{1,2})/); if (m0) return m0[1]; } } catch (e) { /* metadata field unavailable */ } var all2 = mdAll(); for (var i4 = 0; i4 < all2.length; i4++) { var str3 = all2[i4]; var m2 = str3.match(/(\d{1,2})\s*bit/i); if (m2) return m2[1]; } return ""; }
    function computeTrackNumberStr() { var v = playerProp(["trackNumber","xesam:trackNumber"]); var s = toFlatString(v); if (s) return String(s); try { if (fileAudioMeta && fileAudioMeta.tags && fileAudioMeta.tags.track) return String(fileAudioMeta.tags.track); } catch (e) { /* metadata field unavailable */ } return ""; }
    function computeDiscNumberStr() { var v = playerProp(["discNumber","xesam:discNumber"]); var s = toFlatString(v); if (s) return String(s); try { if (fileAudioMeta && fileAudioMeta.tags && fileAudioMeta.tags.disc) return String(fileAudioMeta.tags.disc); } catch (e) { /* metadata field unavailable */ } return ""; }
    function computeAlbumArtist() { var v = playerProp(["albumArtist","xesam:albumArtist"]); var s = toFlatString(v); if (s) return s; try { if (fileAudioMeta && fileAudioMeta.tags && fileAudioMeta.tags.album_artist) return toFlatString(fileAudioMeta.tags.album_artist); } catch (e) { /* metadata field unavailable */ } return ""; }
    function computeComposer() { var v = playerProp(["composer","xesam:composer"]); var s = toFlatString(v); if (s) return s; try { if (fileAudioMeta && fileAudioMeta.tags && fileAudioMeta.tags.composer) return toFlatString(fileAudioMeta.tags.composer); } catch (e) { /* metadata field unavailable */ } return ""; }
    function computeUrlStr() { var v = playerProp(["url","xesam:url"]); var s = toFlatString(v); if (!s) return ""; try { if (s.startsWith("file://")) { return decodeURIComponent(s.replace(/^file:\/\//, "")); } } catch (e) { /* metadata field unavailable */ } return s; }
    function computeDsdVariant(codec, sampleRateStr) { try { if (!codec) return ""; var c = String(codec).toUpperCase(); if (c.indexOf('DSD') === -1) return ""; var hz = parseRateToHz(sampleRateStr || trackSampleRateStr || ""); if (!isNaN(hz) && hz > 0) { var base = 44100; var ratio = hz / base; var candidates = [64, 128, 256, 512, 1024]; var best = 0, bestDiff = 1e9; for (var i = 0; i < candidates.length; i++) { var r = candidates[i]; var diff = Math.abs(ratio - r); if (diff < bestDiff) { bestDiff = diff; best = r; } } if (best > 0 && (bestDiff / best) <= 0.05) { return 'DSD' + best; } } var all3 = mdAll(); for (var j = 0; j < all3.length; j++) { var s3 = String(all3[j]); var m = s3.match(/DSD\s*(64|128|256|512|1024)/i); if (m) return 'DSD' + m[1]; } return 'DSD'; } catch (e) { console.warn("[MusicMeta.computeDsdVariant]", e); return 'DSD'; } }
    function computeDsdRateStr() { try { var codec = trackCodec ? String(trackCodec).toUpperCase() : ""; if (codec.indexOf('DSD') === -1) return ""; var hz = parseRateToHz(trackSampleRateStr); if (!isNaN(hz) && hz > 0) return fmtMHz(hz); var variant = computeDsdVariant(codec, trackSampleRateStr); var m = String(variant).match(/DSD(64|128|256|512|1024)/); if (m) { var mult = Number(m[1]); var estHz = mult * 44100; return fmtMHz(estHz); } var all = mdAll(); for (var j = 0; j < all.length; j++) { var s = String(all[j]); var mhz = s.match(/(\d+(?:\.\d+)?)\s*MHz/i); if (mhz) return mhz[1] + 'M'; var khz = s.match(/(\d{4,6})\s*Hz/i); if (khz) return fmtMHz(khz[1]); } } catch (e) { console.warn("[MusicMeta.computeDsdRateStr]", e); } return ""; }
    function computeDateStr() { var v = playerProp(["date","xesam:contentCreated","xesam:date","xesam:contentcreated"]); var s = toFlatString(v); if (s) return s; try { if (fileAudioMeta && fileAudioMeta.tags && fileAudioMeta.tags.date) return toFlatString(fileAudioMeta.tags.date); } catch (e) { /* metadata field unavailable */ } return ""; }
    function computeContainer() { try { if (fileAudioMeta && fileAudioMeta.container) return String(fileAudioMeta.container).toUpperCase(); } catch (e) { /* metadata field unavailable */ } return ""; }
    ...
    function computeFileSizeStr() { try { if (fileAudioMeta && fileAudioMeta.fileSizeBytes) return fmtBytes(fileAudioMeta.fileSizeBytes); } catch (e) { /* metadata field unavailable */ } return ""; }
    function computeChannelLayout() { try { if (fileAudioMeta && fileAudioMeta.channelLayout) return String(fileAudioMeta.channelLayout); } catch (e) { /* metadata field unavailable */ } return ""; }
    ...
    function pathFromUrl(u) { if (!u) return ""; var s = String(u); if (s.startsWith("file://")) { try { return decodeURIComponent(s.replace(/^file:\/\//, "")); } catch (e) { console.warn("[MusicMeta.pathFromUrl]", e); return s.replace(/^file:\/\//, ""); } } if (s.startsWith("/")) return s; return ""; }
    function isBusy() {
    ...
    function parseFfprobe(obj) {
        ...
        } catch (e) { console.warn("[MusicMeta.parseFfprobe.streams]", e) }
        ...
        try { out.codec = (audio && audio.codec_name) || ""; const fmtname = (fmt.format_name || "").split(',')[0]; if (!out.codec && fmtname) out.codec = fmtname; } catch (e) { /* field not present */ }
        try { out.codecLong = (audio && audio.codec_long_name) || ""; } catch (e) { /* field not present */ }
        try { out.profile = (audio && (audio.profile || audio.profile_name)) || ""; } catch (e) { /* field not present */ }
        try { out.sampleFormat = (audio && (audio.sample_fmt || audio.sample_format)) || ""; } catch (e) { /* field not present */ }
        try { const sr = (audio && audio.sample_rate) ? Number(audio.sample_rate) : (fmt.sample_rate ? Number(fmt.sample_rate) : NaN); if (!isNaN(sr)) out.sampleRate = fmtKHz(sr); } catch (e) { /* field not present */ }
        try { const br = (audio && audio.bit_rate) || fmt.bit_rate || ""; if (br) out.bitrateKbps = fmtKbps(br); } catch (e) { /* field not present */ }
        try { const ch = (audio && audio.channels) || 0; if (ch === 1) out.channels = "Mono"; else if (ch === 2) out.channels = "Stereo"; else if (ch > 2) out.channels = String(ch); } catch (e) { /* field not present */ }
        try { const bps = (audio && (audio.bits_per_raw_sample || audio.bits_per_sample)) || ""; if (bps) out.bitDepth = String(bps); } catch (e) { /* field not present */ }
        try { const sr2 = (fmt.sample_rate ? Number(fmt.sample_rate) : NaN); if (!isNaN(sr2) && !out.sampleRate) out.sampleRate = fmtKHz(sr2); } catch (e) { /* field not present */ }
        try { out.container = (fmt.format_name || "").split(',')[0].toUpperCase(); } catch (e) { /* field not present */ }
        try { out.fileSizeBytes = (fmt.size ? Number(fmt.size) : 0) || 0; } catch (e) { /* field not present */ }
        try { const tags = (fmt.tags || {}); out.tags = tags; } catch (e) { /* field not present */ }
        try { if (audio && audio.channel_layout) out.channelLayout = String(audio.channel_layout); } catch (e) { /* field not present */ }
        try { if (fmt && fmt.encoder) out.encoder = String(fmt.encoder); } catch (e) { /* field not present */ }
        return out;
    }
    function parseMediainfo(obj) {
        try {
            ...
            out.tags = tags;
            return out;
        } catch (e) { console.warn("[MusicMeta.parseMediainfo]", e); return null; }
    }
    function parseSoxInfo(text) {
        try {
            ...
            return out;
        } catch (e) { console.warn("[MusicMeta.parseSoxInfo]", e); return null; }
    }
}
