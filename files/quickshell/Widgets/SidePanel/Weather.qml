    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Canvas { id: sunIcon; visible: !isLoading; width: Math.round(Theme.uiIconSizeLarge*2.0*Theme.scale(Screen)*weatherRoot.wscale); height: width; Layout.alignment: Qt.AlignVCenter; onPaint: {var ctx=getContext("2d");ctx.reset();var s=width,r=s*0.30;var cx=s/2,cy=s/2;var t=Date.now()*0.001;var grd=ctx.createRadialGradient(cx,cy,r*0.05,cx,cy,r*1.5);grd.addColorStop(0,"rgba(255,255,255,1)");grd.addColorStop(0.12,"rgba(255,255,240,0.95)");grd.addColorStop(0.35,Color.withAlpha(Theme.accentPrimary,0.5));grd.addColorStop(0.65,Color.withAlpha(Theme.accentPrimary,0.1));grd.addColorStop(1,"transparent");ctx.fillStyle=grd;ctx.beginPath();ctx.arc(cx,cy,r*1.5,0,Math.PI*2);ctx.fill();ctx.globalCompositeOperation="lighter";for(var i=0;i<900;i++){var a=Math.PI*2*i/900;var wig=Math.sin(i*4.7+t*0.8)*0.35+Math.sin(i*13.3-t*0.5)*0.2+Math.sin(i*21.1+t*1.1)*0.12;var len=r*(0.5+wig);var al=0.15+Math.abs(wig)*0.7;ctx.globalAlpha=al*0.3;ctx.strokeStyle=Theme.accentPrimary;ctx.lineWidth=0.4+Math.abs(Math.sin(i*23.7+t*3.1))*1.0;ctx.beginPath();ctx.moveTo(cx+Math.cos(a)*r*0.25,cy+Math.sin(a)*r*0.25);ctx.lineTo(cx+Math.cos(a)*len,cy+Math.sin(a)*len);ctx.stroke()}ctx.globalCompositeOperation="source-over"} Timer { interval: 33; repeat: true; running: true; onTriggered: sunIcon.requestPaint() } }
