    ...
    Rectangle {
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Canvas { id: sunIcon; visible: !isLoading; width: Math.round(Theme.uiIconSizeLarge*1.1*Theme.scale(Screen)*weatherRoot.wscale); height: width; Layout.alignment: Qt.AlignVCenter; onPaint: {var ctx=getContext("2d");ctx.reset();var s=width,r=s*0.12;var cx=s/2,cy=s/2;var grd=ctx.createRadialGradient(cx,cy,r*0.1,cx,cy,r*1.3);grd.addColorStop(0,"rgba(255,255,255,1)");grd.addColorStop(0.2,"rgba(255,255,240,0.9)");grd.addColorStop(0.5,Color.withAlpha(Theme.accentPrimary,0.45));grd.addColorStop(0.8,Color.withAlpha(Theme.accentPrimary,0.08));grd.addColorStop(1,"transparent");ctx.fillStyle=grd;ctx.beginPath();ctx.arc(cx,cy,r*1.3,0,Math.PI*2);ctx.fill();ctx.globalCompositeOperation="lighter";for(var i=0;i<90;i++){var a=Math.PI*2*i/90;var len=r*(0.7+Math.abs(Math.sin(i*5.3))*0.5);ctx.globalAlpha=0.15+Math.abs(Math.sin(i*7.7))*0.2;ctx.strokeStyle=Theme.accentPrimary;ctx.lineWidth=0.5+Math.abs(Math.sin(i*19.3))*1.2;ctx.beginPath();ctx.moveTo(cx+Math.cos(a)*r*0.4,cy+Math.sin(a)*r*0.4);ctx.lineTo(cx+Math.cos(a)*len,cy+Math.sin(a)*len);ctx.stroke()}ctx.globalCompositeOperation="source-over"} Timer { interval: 50; repeat: true; running: true; onTriggered: sunIcon.requestPaint() }
