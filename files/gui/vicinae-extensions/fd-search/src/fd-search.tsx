...
type CachedIndexData = {
  // Compact storage: raw index lines only. Avoids building 847k+ entry
  // objects and a huge prefix map (OOM). Search lowercases on the fly.
  lines: string[];
};
...
function getCandidateEntries(
  ...
  // Returns indices into cachedIndexData.lines (not parsed entries) so the
  // caller can parse only the candidates. Linear scan replaces the huge
  // prefixIndex map — for ~850k lines an includes() pass is ~tens of ms.
  const lines = cachedIndexData.lines;
  if (!lines.length) {
    return [];
  }
    ...
    cachedIndexPromiseRef.current = (async () => {
      ...
      try {
        ...
        for await (const line of rl) {
          ...
          // Store raw line; parse into an entry object lazily (only for
          // candidates) in runSearch to keep memory low.
          cachedIndexData.lines.push(line);
  ...
  async function runSearch(q: string) {
    ...
    try {
      ...
      for (const index of candidateEntries) {
        ...
        // Lazily parse only the candidate lines into entry objects.
        const entry = parseIndexedLine(cachedIndexData.lines[index]);
        if (!entry) {
          continue;
        }
