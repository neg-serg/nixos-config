      ...
      types.submodule (
        ...
        in
        {
          ...
          config = mkIf cfg.enable {
            ...
            systemd.services."qemu-vm-${cfg.name}" =
              let
                qemuBin = lib.getExe' (cfg.qemuPackage or pkgs.qemu_kvm) "qemu-system-x86_64";

                netArgs =
                  if cfg.networkBackend == "bridge" then
                    [
                      "-device"
                ...
                cmd = qemuBin + " " + lib.concatStringsSep " " baseArgs;

                gracefulShutdownScript = pkgs.writeShellScript "qemu-vm-${cfg.name}-shutdown" ''
                  #!/bin/sh
                  sock='${monitorPath}'
                  if [ -S "$sock" ]; then
