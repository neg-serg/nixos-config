...
def get_hyprctl_json(cmd_type):
    try:
        result = subprocess.run(
            ["hyprctl", "-j", cmd_type], capture_output=True, text=True, check=True
        )
        ...
        print(f"Error parsing JSON from hyprctl -j {cmd_type}: {e}", file=sys.stderr)
        sys.exit(1)
...
def get_config_rules():
    ...
    try:
        ...
        for line in output.splitlines():
            ...
            workspace_id = None
            pattern = None
            rule_type = None # 'class', 'title', 'initialClass', 'initialTitle'
            
            # Simple parsing strategy: look for "workspace" action
            if "workspace" in line:
                # Handle windowrulev2 format: "windowrulev2: workspace 2 silent, class:^(firefox)$"
                # Handle windowrule format: "windowrule: workspace 2, ^(firefox)$" -> implies class
                
                parts = line.split(":", 1)
                if len(parts) < 2:
                    continue
                    
                rule_def = parts[1].strip() # "workspace 2 silent, class:^(firefox)$"
                ...
                if condition_part.startswith("class:"):
                    ...
                    # windowrule (v1) implies class matched against regex if just regex
                    # "workspace 2, ^(firefox)$"
                    rule_type = "class"
...
def main():
    ...
    for client in clients:
        ...
        # Skip special workspaces? (usually negative or huge numbers? or named 'special:...')
        ...
        if target_ws:
            ...
            # Normalize target_ws: remove "name:" if present to match against ID or name?
            # Actually hyprctl dispatch movetoworkspace accepts "ID" or "name:X"
            ...
            if target_simple != current_ws_id and target_simple != current_ws_name:
                 # Additional check: "name:web" vs "web"
                if target_simple.startswith("name:") and target_simple[5:] == current_ws_name:
                    continue
                    
                print(f"Match: {client['class']} ({client['title'][:20]}...) -> Rule target: {target_ws} (Current: {current_ws_name})")
