...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h | --help) usage ;;
    -*)
      echo "ai-upscale-video: unknown option: $1" >&2
      usage
      ;;
    *)
      files+=("$1")
...
[ ${#files[@]} -ge 1 ] || usage
[ -n "$out_opt" ] && [ ${#files[@]} -gt 1 ] && {
  echo "ai-upscale-video: -o works with a single input only" >&2
  exit 2
}
...
for tool in ffmpeg ffprobe vspipe; do
  command -v "$tool" > /dev/null || {
    echo "ai-upscale-video: missing dependency: $tool" >&2
    exit 3
  }
done
[ -f "$ONNX" ] || {
  echo "ai-upscale-video: model not found: $ONNX" >&2
  exit 3
}
...
upscale_one() {
  ...
  in="$(readlink -f "$1")"
  [ -f "$in" ] || {
    echo "ai-upscale-video: not a file: $in" >&2
    return 1
  }
  ...
  [ "$in" = "$out" ] && {
    echo "ai-upscale-video: input == output, refusing" >&2
    return 1
  }
...
PY
  ...
  vspid=$!
  sleep 1
  kill -0 "$vspid" 2> /dev/null || {
    cat "$work/vspipe.log" >&2 || true
    wait "$vspid" || true
    return 1
  }
...
fail=0
for f in "${files[@]}"; do upscale_one "$f" || fail=1; done
exit $fail
