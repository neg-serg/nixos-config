...
#     --scale N    output scale (default 4; 2 = half-size output)
...
command -v realesrgan-ncnn-vulkan > /dev/null || {
  echo "upscale-image: realesrgan-ncnn-vulkan not found" >&2
  exit 3
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h | --help)
      ...
      ;;
    -*)
      echo "upscale-image: unknown option: $1" >&2
      exit 2
      ;;
    *)
      files+=("$1")
...
[ ${#files[@]} -gt 0 ] || {
  echo "upscale-image: no inputs" >&2
  exit 2
}
...
upscale_file() {
  ...
  in="$(readlink -f "$1")"
  [ -f "$in" ] || {
    echo "upscale-image: not a file: $in" >&2
    return 1
  }
  if [ -n "$out_opt" ]; then out="$(readlink -f "$out_opt")"; else
    stem="$(basename "$in" | sed 's/\.[^.]*$//')"
    dir="$(dirname "$in")"
    out="$dir/${stem}_x${SCALE}.png"
  fi
  [ "$in" = "$out" ] && {
    echo "upscale-image: input == output, refusing" >&2
    return 1
  }
...
if [ ${#files[@]} -eq 1 ] && [ -d "${files[0]}" ]; then
  in_dir="$(readlink -f "${files[0]}")"
  if [ -n "$out_opt" ]; then out_dir="$(readlink -f "$out_opt")"; else
    out_dir="$(dirname "$in_dir")/$(basename "$in_dir")_x${SCALE}"
  fi
  mkdir -p "$out_dir"
  ...
  fail=0
  for f in "${files[@]}"; do upscale_file "$f" || fail=1; done
  exit $fail
fi
