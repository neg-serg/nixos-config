...
let
  ...
  # dsh-hub: supervised long-running processes (REPL/watcher/dev-server).
  ...
  # ~/.dsh/profiles/web/node_modules/dsh-hub and restart dsh.
  pkg = ./dsh-hub;

  ensure = pkgs.writeShellScript "dsh-hub-ensure" ''
        set -eu
        export PATH=/run/current-system/sw/bin:$PATH
        PROFILE_DIR="${homeDir}/.dsh/profiles/web"
        P="$PROFILE_DIR/node_modules/dsh-hub"
        mkdir -p "$P/lib"
        changed=0
        ...
        if ! grep -q 'dsh-hub' "$PATCH" 2>/dev/null; then
          cat >> "$PATCH" <<'YAML'

    # dsh-hub - supervised long-running processes
    # (module: dsh-hub.nix).
    - insert:
        - id: hub
          name: dsh-hub
...
in
{
  ...
  system.activationScripts.dshHub = lib.stringAfter [ "users" "dshMarketEnsure" ] ''
  ...
  systemd.user.services.dsh-hub = {
    enable = true;
    description = "dsh-hub — ensure supervised-process tool in the dsh web profile";
    after = [
      "network.target"
