...
const RISK_ZH = { SAFE: "Safe", LOW: "Low", MEDIUM: "Medium", HIGH: "High", ALLOWED: "Allowlisted" };
...
function renderPackage(p) {
	...
	lines.push(`[${tag}] ${p.name}@${p.version} (score ${p.score})${p.needsReview === true ? " [REVIEW: dynamic code execution]" : ""}`);
...
function renderReport(report) {
	...
	lines.push(`safe=${report.summary.safe} low=${report.summary.low} medium=${report.summary.medium} high=${report.summary.high} (safe/low/medium/high)`);
