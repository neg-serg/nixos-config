...
let
  ...
  # compaction (server plugin, host plane — same pattern as dsh-osm). The
  ...
  # ~/.dsh/profiles/web/node_modules/dsh-compaction-todo-preserver and
  # restart dsh.
  pkg = ./dsh-compaction-todo-preserver;

  ensure = pkgs.writeShellScript "dsh-compaction-todo-preserver-ensure" ''
        set -eu
        export PATH=/run/current-system/sw/bin:$PATH
        PROFILE_DIR="${homeDir}/.dsh/profiles/web"
        P="$PROFILE_DIR/node_modules/dsh-compaction-todo-preserver"
        mkdir -p "$P/lib"
        changed=0
        ...
        if ! grep -q 'dsh-compaction-todo-preserver' "$PATCH" 2>/dev/null; then
          cat >> "$PATCH" <<'YAML'

    # dsh-compaction-todo-preserver - re-append todo list after compaction
    # (module: dsh-compaction-todo-preserver.nix).
    - insert:
...
in
{
  ...
  system.activationScripts.dshCompactionTodoPreserver =
    lib.stringAfter [ "users" "dshMarketEnsure" ]
      ''
  ...
  systemd.user.services.dsh-compaction-todo-preserver = {
    enable = true;
    description = "dsh-compaction-todo-preserver — ensure todo-preservation plugin in the dsh web profile";
    after = [
      "network.target"
