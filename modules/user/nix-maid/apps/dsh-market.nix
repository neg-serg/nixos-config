...
let
  ...
  ensureMarket = pkgs.writeShellScript "dsh-market-ensure" ''
    ...
    YAML
            ...
            # Canonical source of the theme is the dsh-web-ui fork checkout —
            # link it when present, so reinstalls never fall back to the stale
            # embedded copy below.
            TUI_FORK="$HOME/src/1st-level/@projects/dsh-web-ui/packages/dsh-terminal-ui"
            if [ ! -e "$TUI" ] && [ -d "$TUI_FORK" ]; then
              ln -s "$TUI_FORK" "$TUI"
            fi
    ...
    /* terminal input: 14px to match the message flow. Caret left stock — the
       green tint was removed (native caret keeps the app default color). */
    .uV2eYG_input {
      font-size: 14px;
    }
    ...
    YAML
            ...
            # non-URL image sources to this route, so `![name](path)` previews
            # local images in assistant messages and thoughts. Image content
            # types only; paths must resolve inside an allowed root (workspace
            # registry + home + tmp). Plain directory like dsh-terminal-ui —
            # pnpm is intentionally not used (the @deepseek-ai store symlink
            # makes pnpm writes fail with EROFS, see dshAiStore). Files are
            # written only when missing, so local tweaks survive.
            PREVIEW="$PROFILE_DIR/node_modules/dsh-preview"
            if [ ! -f "$PREVIEW/package.json" ] || [ ! -f "$PREVIEW/lib/index.js" ]; then
              mkdir -p "$PREVIEW/lib"
