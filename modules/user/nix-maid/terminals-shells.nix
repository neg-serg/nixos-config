{
  ...
  kittyPanel = pkgs.writeShellScriptBin "kitty-panel" ''
    ...
    while true; do
      ...
      color24=$(read_color color24 '#5fb0f6')

      if [[ "$packages$volume$brightness$network$color0$color24" != "$prev_packages$prev_volume$prev_brightness$prev_network$prev_c0$prev_c24" ]]; then
        ...
        packages_box=$(gum style --border=rounded --border-foreground="$color24" --padding "0 1" "Packages: $packages")
        volume_box=$(gum style --border=rounded --border-foreground="$color24" --padding "0 1" "Volume: $volume")
        brightness_box=$(gum style --border=rounded --border-foreground="$color24" --padding "0 1" "Brightness: $brightness")
        network_box=$(gum style --border=rounded --border-foreground="$color24" --padding "0 1" "Wi-Fi: $network")
  ...
  # Kitty Scrollback wrapper
  nixKsbPath = "${pkgs.vimPlugins.kitty-scrollback-nvim}/python/kitty_scrollback_nvim.py";
  kittyScrollback = pkgs.writeScriptBin "kitty-scrollback-nvim" ''
    #!/usr/bin/env python3
    import os
    import sys
...
in {
  ...
  users.users.neg.maid.file.home = {
    # Kitty Config
    ".config/kitty".source = impurity.link "/etc/nixos/files/kitty";

    # Tmux Config
    ".config/tmux".source = impurity.link "/etc/nixos/files/tmux";

    # Nushell Config
    ".config/nushell/aliases.nu".source = impurity.link "/etc/nixos/files/nushell/aliases.nu";
    ".config/nushell/git.nu".source = impurity.link "/etc/nixos/files/nushell/git.nu";
    ".config/nushell/broot.nu".source = impurity.link "/etc/nixos/files/nushell/broot.nu";
    ".config/nushell/git-completion.nu".source = impurity.link "/etc/nixos/files/nushell/git-completion.nu";
    ...
    ".config/nushell/config.nu".source = impurity.link "/etc/nixos/files/nushell/config.nu";
    ...
    ".config/nushell/env.nu".text =
      ...
      + ''
        ...
        # -- Generated by NixOS --
        $env.PROMPT_COMMAND = {||
          ${pkgs.oh-my-posh}/bin/oh-my-posh print primary --shell nu --config=${impurity.link "/etc/nixos/files/shell/zsh/neg.omp.json"}
        }
