...
- Follow the layout in `modules/README.md`: put new modules in the appropriate domain folder and add
  them to that domain's `modules.nix` (not `default.nix`).
- Define new options in `features.nix` and refresh option docs (`OPTIONS.md`/generated outputs) when
  behavior changes.
