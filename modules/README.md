...
When adding a new module in a domain, include it in that domain's `modules.nix` rather than editing `default.nix`.

При добавлении нового модуля в домен, добавляйте его в `modules.nix` этого домена, а не редактируйте `default.nix`.
