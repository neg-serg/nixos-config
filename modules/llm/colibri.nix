...
let
  ...
  mkColibri =
    ...
    pkgs.stdenv.mkDerivation {
      ...
      buildPhase = ''
        runHook preBuild
        # `glm` is a phony make alias for the real `colibri` binary; build the
        # explicit target to avoid the circular-dependency warning.
      ...
      installPhase =
        builtins.replaceStrings
          [ "@PYTHONENV@" "@PYTHONENV_V2@" "@SITEPACKAGES@" ]
          [ "${pythonEnv}" "${pythonEnv}" "${pkgs.python3.sitePackages}" ]
          (builtins.readFile ./colibri-install.sh);
