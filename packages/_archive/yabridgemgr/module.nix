...
let
  ...
  mkBool =
    if (config ? lib) && (config.lib ? neg) && (config.lib.neg ? mkBool) then
      config.lib.neg.mkBool
    else
      (desc: default: (lib.mkEnableOption desc) // { inherit default; });
...
in
{
  options.modules.audio-nix.yabridgemgr = {
    ...
    plugins = mkOption {
      ...
      default = [
        pkgs.neg.yabridgemgr.plugins.valhalla_supermassive # Valhalla Supermassive (VST)
        pkgs.neg.yabridgemgr.plugins.voxengo_span # Voxengo SPAN analyzer (VST)
        pkgs.neg.yabridgemgr.plugins.piz_midichordanalyzer # MIDI chord analyzer (VST)
      ];
        ...
        lib.recursiveUpdate
          {
            ...
            Service = {
              ...
              ExecStart =
                let
                  exe = "${mount_prefix}/bin/mount_prefix";
                in
                "${exe}";
              ExecStop =
                let
                  exe = "${umount_prefix}/bin/umount_prefix";
                in
                "${exe}";
              RemainAfterExit = "yes";
            };
