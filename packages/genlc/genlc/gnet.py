...
def _gsm16(data: bytes) -> int:
    """CRC-16/GSM checksum (poly=0x1021, init=0x0000, xorout=0xFFFF)."""
    crc = 0x0000
    for byte in data:
        crc ^= byte << 8
        for _ in range(8):
...
class GNetMessage:
    ...
    def __repr__(self) -> str:
        return f"GNetMessage({hex(self.address)}, {hex(self.command)}, {repr(self.data)}"
...
class GNetResponseMessage:
    ...
    def parse(self) -> None:
        ...
        calculated_checksum = _gsm16(self.msg[:-3])
