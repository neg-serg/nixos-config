...
def get_samgroup() -> sam.SAMGroup:
    ...
    try:
        hid_glm_adapter = hid.Device(
            const.GENELEC_GLM_VID, const.GENELEC_GLM_PID
        )
...
def discover():
    ...
    try:
        ...
        raise click.ClickException(
            f"Timeout communicating with the GLM adapter: {te}"
        )
...
@click.option(
    "--continuous", is_flag=True, default=False, help="Continuous polling"
)
@click.option(
    "--interval",
    type=click.FLOAT,
    default=1.0,
    help="Repeat every FLOAT seconds",
)
            ...
            try:
                ...
                click.echo(
                    ...
                    + ", ".join(
                        (
                            f"{f}: {v:.3f}"
                            for f, v in monitor.poll_fields.items()
                        )
                    )
                )
...
@click.option(
    "--mute/--unmute", required=False, default=None, help="Mute/unmute sound"
)
@click.option(
    "--led-color",
