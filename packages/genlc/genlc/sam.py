...
class SAMGroup:
    ...
    def set_volume_glm(self, dB: float) -> None:
        ...
        logger.debug(
            f"volume ({dB} dB): {vol_sint24}\thex: {hex(int(vol_sint24))}"
        )
    ...
    def wakeup_all(self) -> None:
        ...
        for data in [bytes([3, 0x7F]), bytes([3, 1])]:
            msg = GNetMessage(
                const.GNET_BROADCAST_ADDR, const.CID_WAKEUP, data=data
            )
    ...
    def race(self) -> int:
        ...
        try:
            ...
            if len(resp) == 3:
                ...
                raise GNetException(
                    f"Invalid response received to CID_RACE: {resp}"
                )
...
class BaseDevice:
    ...
    def __str__(self) -> str:
        return (
            f"[{self.address}]{self.hardware[0]}#{getattr(self, 'serial', '')}"
        )
    ...
    def query_hardware(self) -> str:
        ...
        hardware_str = resp_msg.response.rstrip(b"\0").decode(
...
class USBAdapter(BaseDevice):
    ...
    def poll(self) -> dict:
        ...
        fields = {
            "microphone_dBSPL": (
                util.dBSPL_from_sint24(spl_raw) if spl_raw > 0 else 0
            ),
        }
...
class SAMMonitor(BaseDevice):
    ...
    def __init__(
        self, address: int, glm: SAMGroup = None, serial: int = None
    ) -> None:
    ...
    def poll(self) -> dict:
        ...
        fields = {
            "temperature": resp[1],
            "input_dBFS": int.from_bytes(
                resp[6:7], byteorder="big", signed=True
            ),
            "output_dBFS": int.from_bytes(
                resp[12:13], byteorder="big", signed=True
            ),
        }
        self.poll_fields.update(fields)
        return fields
