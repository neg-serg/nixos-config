    ...
    def convert(self, value, param, ctx):
        ...
        try:
            if value.endswith("%"):
                ...
                vol_pct = click.FloatRange(
                    min=0.0001, max=100.0, clamp=True
                ).convert(value.replace("%", ""), param, ctx)
            ...
            return click.FloatRange(min=-130.0, max=0.0).convert(
                value, param, ctx
            )
        except ValueError:
...
class MonitorListParamType(click.ParamType):
    ...
    def convert(
        self, value: Union[str, Set], param, ctx
    ) -> Set[Union[int, str]]:
        ...
        try:
            ...
            return {
                id if id[0] == "#" and id[1:].isnumeric() else int(id)
                for id in parts
            }
        except ValueError:
