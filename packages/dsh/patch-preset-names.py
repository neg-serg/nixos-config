...
REWRITES = {
    "code": {
        ...
        "description: 具备标准模式的全部能力，并通过 Code Mode SDK 呈现工具，让模型用一个 TypeScript 程序组合多步操作。":
            "description: All the capabilities of the standard mode; tools are presented through the Code Mode SDK so the model composes multi-step operations in a single TypeScript program.",
    },
    "minimal": {
    ...
    "cordis": {
        ...
        "description: 用于创建自定义 Agent preset：具备标准模式的全部能力，并提供运行时检查、插件实验和 preset 创作指导。":
            "description: For authoring custom agent presets: all standard capabilities plus runtime inspection, plugin experimentation, and preset-authoring guidance.",
    },
}
...
print("patch-preset-names: rewrote shipped preset names/descriptions to English")
