...
in
{
  ...
  # tarball (relative-path pattern, see the vendored-tarball note in
  ...
  neg = rec {
    ...
    niri-screen-time = callPkg (packagesRoot + "/niri-screen-time") { }; # Screen time tracker by window class/title (niri + Hyprland daemon and report CLI)
