...
in
{
  ...
  gjs = prev.gjs.overrideAttrs (_: {
    doCheck = false;
  });

  # Limit parallelism for OOM-prone builds
  webkitgtk_4_1 = prev.webkitgtk_4_1.overrideAttrs (_: {
    NIX_BUILD_CORES = 4;
  });
  qt6 = prev.qt6 // {
  ...
  kdePackages = prev.kdePackages // {
    kcoreaddons = prev.kdePackages.kcoreaddons.overrideAttrs (_: {
      NIX_BUILD_CORES = 4;
    });
    kguiaddons = prev.kdePackages.kguiaddons.overrideAttrs (_: {
      NIX_BUILD_CORES = 4;
    });
    knotifications = prev.kdePackages.knotifications.overrideAttrs (_: {
      NIX_BUILD_CORES = 4;
    });
    kwidgetsaddons = prev.kdePackages.kwidgetsaddons.overrideAttrs (_: {
      NIX_BUILD_CORES = 4;
    });
    kauth = prev.kdePackages.kauth.overrideAttrs (_: {
      NIX_BUILD_CORES = 4;
    });
    "kirigami-addons" = prev.kdePackages."kirigami-addons".overrideAttrs (_: {
      NIX_BUILD_CORES = 4;
    });
    kdeconnect-kde = prev.kdePackages.kdeconnect-kde.overrideAttrs (_: {
      NIX_BUILD_CORES = 4;
    });
  };
